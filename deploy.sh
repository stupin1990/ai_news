...
if [ -z "$APP_KEY_VALUE" ]; then
  ...
  docker compose exec -T "$APP_SERVICE" php artisan key:generate --force --no-interaction
else
...
docker compose exec -T "$APP_SERVICE" php artisan db:seed --force --no-interaction
...
docker compose exec -T "$APP_SERVICE" php artisan optimize:clear --no-interaction
docker compose exec -T "$APP_SERVICE" php artisan optimize --no-interaction
docker compose exec -T "$APP_SERVICE" php artisan queue:restart --no-interaction
