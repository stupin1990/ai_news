...
<div class="flex min-h-screen items-center justify-center">
    <div class="p-8">
        ...
        {{-- Main buttons --}}
        <div id="auth-buttons" class="flex w-fit flex-col gap-3" style="display: none;">
            ...
            <button type="button" onclick="showView('password')"
                    class="text-center cursor-pointer text-sm text-gray-500 hover:text-gray-700 dark:text-[#A1A09A] dark:hover:text-[#EDEDEC]">
                Forgot your password ?
            </button>
        </div>

        {{-- Email login form --}}
        <form id="email-form" method="POST" action="{{ route('login.submit') }}" class="w-fit flex-col gap-4" style="display: none;">
            @csrf
            <div>
                <label for="login-email" class="mb-1 block text-sm font-medium">Email</label>
                <input id="login-email" name="email" type="email" value="{{ old('email') }}" autocomplete="new-password" required
            ...
            <div>
                ...
                <input id="login-password" name="password" type="password" autocomplete="new-password" required
                       class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-[#3E3E3A] dark:bg-[#161615]">
            </div>
            <div class="flex items-center gap-3 mt-2">
                ...
                <button type="button" onclick="showView('buttons')"
                        class="cursor-pointer text-sm text-gray-500 hover:text-gray-700 dark:text-[#A1A09A] dark:hover:text-[#EDEDEC]">
                    &larr; Back
                </button>
            </div>
        </form>
        ...
        {{-- Register form --}}
        <form id="register-form" method="POST" action="{{ route('register.submit') }}" class="w-fit flex-col gap-4" style="display: none;">
            @csrf
            <div>
                <label for="reg-name" class="mb-1 block text-sm font-medium">Name</label>
                <input id="reg-name" name="name" type="text" value="{{ old('name') }}" required
            ...
            <div>
                <label for="reg-email" class="mb-1 block text-sm font-medium">Email</label>
                <input id="reg-email" name="email" type="email" value="{{ old('email') }}" autocomplete="new-password" required
                       class="w-[300px] rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-[#3E3E3A] dark:bg-[#161615]">
            </div>
            <div>
                <label for="reg-password" class="mb-1 block text-sm font-medium">Password</label>
        ...
        {{-- Password form --}}
        <form id="password-form" method="POST" action="{{ route('password.submit') }}" class="w-fit flex-col gap-4" style="display: none;">
            @csrf
            <div>
                <label for="reg-email-pass" class="mb-1 block text-sm font-medium">Email</label>
                <input id="reg-email-pass" name="email" type="email" autocomplete="new-password" required
                       class="w-[300px] rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-[#3E3E3A] dark:bg-[#161615]">
            </div>
            <div class="flex items-center gap-3 mt-2">
                <button type="submit"
...
<script>
    const views = {
        buttons: { el: 'auth-buttons', url: '/login' },
        email:   { el: 'email-form',   url: '/login/email' },
        register:{ el: 'register-form', url: '/register' },
        password:{ el: 'password-form', url: '/password' },
    };

    function showView(name) {
        Object.values(views).forEach(v => {
            document.getElementById(v.el).style.display = 'none';
        });
        const view = views[name];
        document.getElementById(view.el).style.display = 'flex';
        history.pushState(null, '', view.url);
    }

    // Show correct view on page load based on server-side variable
    ...
    window.addEventListener('popstate', function () {
        ...
        else if (path === '/login/email') showView('email');
        else if (path === '/password') showView('password');
        else showView('buttons');
    });
</script>
@endsection
